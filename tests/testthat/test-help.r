...
test_that("devtools::shim_help behaves the same as utils::help for non-devtools-loaded packages", {
  ...
  expect_identical(devtools::shim_help(lm)[1],            utils::help(lm)[1])
  expect_identical(devtools::shim_help(lm, stats)[1],     utils::help(lm, stats)[1])
  expect_identical(devtools::shim_help(lm, 'stats')[1],   utils::help(lm, 'stats')[1])
  expect_identical(devtools::shim_help('lm')[1],          utils::help('lm')[1])
  expect_identical(devtools::shim_help('lm', stats)[1],   utils::help('lm', stats)[1])
  expect_identical(devtools::shim_help('lm', 'stats')[1], utils::help('lm', 'stats')[1])
...
test_that("devtools::help behaves the same as utils::help for nonexistent objects", {
  expect_equal(length(devtools::shim_help(foofoo)), 0)
  expect_equal(length(devtools::shim_help("foofoo")), 0)
})


test_that("devtools::? behaves the same as utils::? for non-devtools-loaded packages", {
  expect_identical(devtools::shim_question(lm)[1], utils::`?`(lm)[1])
  expect_identical(devtools::shim_question(`lm`)[1], utils::`?`(`lm`)[1])
  expect_identical(devtools::shim_question('lm')[1], utils::`?`('lm')[1])
})

test_that("devtools::? behaves the same as utils::? for nonexistent objects", {
  expect_equal(length(devtools::shim_question(foofoo)), 0)
  expect_equal(length(devtools::shim_question(`foofoo`)), 0)
  expect_equal(length(devtools::shim_question("foofoo")), 0)
})
