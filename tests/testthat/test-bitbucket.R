...
test_that("Bitbucket remote build works", {
  ...
  mock_resolve_ref.bitbucket_pull <- function(x, params) {
    params$username <- sprintf("user-%s", x)
    params$ref <- sprintf("pull-%s", x)
    params
  }
  with_mock(resolve_ref.bitbucket_pull = mock_resolve_ref.bitbucket_pull, {
    ...
    expect_equal(bitbucket_remote("imanuelcostigan/devtestprivate#1")$ref, "pull-1")
