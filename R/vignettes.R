...
build_vignettes <- function(pkg = ".",
  ...
  glue_inform("Building {pkg$package} vignettes")
...
create_vignette_index <- function(pkg, vigns) {
  ...
  glue_inform("Building vignette index")
...
clean_vignettes <- function(pkg = ".") {
  ...
  glue_inform("Cleaning built vignettes and index from {pkg$package}")
  ...
  if (length(to_remove) > 0) {
    glue_inform("Removing {paste(basename(to_remove), collapse = ', ')}")
